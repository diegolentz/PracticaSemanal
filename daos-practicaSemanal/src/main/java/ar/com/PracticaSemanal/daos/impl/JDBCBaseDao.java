...
import ar.com.PracticaSemanal.daos.GenericDao;
...
import ar.com.PracticaSemanal.domain.Entity;
...
public abstract class JDBCBaseDao<T extends Entity> implements GenericDao<T>{
	...
	public String getSaveSQL2(T entity) throws GenericException {
		...
		try {
			...
			sb.append("( ");
			for (int i = 1; i < atributos.length; i++) { //salteo el id
				atributos[i].setAccessible(true);
				String fName = atributos[i].getName();// agregar el _ entre separacion
			...
			sb.append(") VALUES (");
			// agrego los ? 
			for (int i = 1; i < atributos.length; i++) { // comienza despues del  ID
				sb.append("?,");
			}
			sb.deleteCharAt(sb.length() - 1); // borra la ultima ","
			sb.append(")");
